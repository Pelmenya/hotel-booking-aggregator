    ...
    async searchHotelsIdx(query: SearchBaseParams): Promise<{ idx: string }[]> {
        ...
        const sql = `
            SELECT DISTINCT h.id AS idx, 
                   ...
                   ts_rank_cd(h.search_vector, to_tsquery('english', $1)) +
                   ts_rank_cd(l.search_vector, to_tsquery('russian', $1)) + 
                   ts_rank_cd(l.search_vector, to_tsquery('english', $1)) +
                   ts_rank_cd(h.search_vector, websearch_to_tsquery('russian', $1)) + 
                   ts_rank_cd(h.search_vector, websearch_to_tsquery('english', $1)) +
                   ts_rank_cd(l.search_vector, websearch_to_tsquery('russian', $1)) + 
            ...
            WHERE 
                (
                    h.search_vector @@ websearch_to_tsquery('russian', $1) 
                    OR h.search_vector @@ websearch_to_tsquery('english', $1) 
                    OR l.search_vector @@ websearch_to_tsquery('russian', $1)
                    OR l.search_vector @@ websearch_to_tsquery('english', $1)
                    OR h.search_vector @@ to_tsquery('russian', $1)
